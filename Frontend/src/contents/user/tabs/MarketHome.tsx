...
export default function MarketHome() {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        setUserMethods(
          methodsArray.map((m: MethodPayment) => ({
  ...
  const estimatedTotal = useMemo(() => {
    ...
    const rate = formData.user_rate
      ? parseFloat(formData.user_rate)
      : marketRate;
  ...
  const fetchListings = useCallback(async () => {
    try {
      setLoading(true);
      const response: ListingPaginationResponse =
        await ListingService.getAllListings(page);
      let filteredData = response.data;

      if (selectedCountry !== "Tous")
        filteredData = filteredData.filter(
          (l) =>
            l.currency_from === selectedCountry ||
            l.currency_to === selectedCountry,
        );
      if (filterFrom !== "Toutes")
        filteredData = filteredData.filter(
          (l) => l.currency_from === filterFrom,
        );
      if (filterTo !== "Toutes")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const finalRate = formData.user_rate
      ? parseFloat(formData.user_rate)
      : marketRate || 0;
    ...
    try {
      const payload: Partial<Listing> = {
        ...
        description:
          ...
          `Échange ${formData.currency_from} → ${formData.currency_to}`,
      };
      ...
      setFormError(
        "Erreur lors de la publication. Vérifiez votre KYC ou votre solde.",
      );
    }
  };
    ...
    <components.Option {...props}>
      <div className="d-flex align-items-center cursor-pointer">
        <img
          src={props.data.flag}
          alt=""
          style={{ width: 22, height: 16, marginRight: 10, borderRadius: 2 }}
        />
        <div>
  ...
  return (
    <div className="market-container w-100 py-4 px-3">
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 px-2">
          <div>
            ...
            <p className="text-muted small mb-0">
              Échangez vos devises en toute sécurité via Escrow
            </p>
          </div>
          <button
            onClick={toggleModal}
            className="btn btn-excha-orange rounded-pill px-4 py-2 fw-bold text-white shadow-lg d-flex align-items-center justify-content-center mt-3 mt-md-0 transition-hover"
          >
        ...
        <div className="bg-white p-3 rounded-4 shadow-sm border mb-4">
          ...
          <div className="d-flex overflow-x-auto no-scrollbar pt-1 gap-2">
            <button
              ...
              className={`filter-pill px-4 py-2 rounded-pill border-0 flex-shrink-0 transition-all ${selectedCountry === "Tous" ? "bg-dark text-white shadow" : "bg-light text-muted"}`}
            >
            ...
            {currencies.map((curr) => (
              <button
                ...
                className={`filter-pill px-3 py-2 rounded-pill border-0 flex-shrink-0 transition-all ${selectedCountry === curr.value ? "bg-excha-orange text-white fw-bold shadow" : "bg-light text-muted border"}`}
              >
                <img
                  src={curr.flag}
                  alt=""
                  className="me-2 rounded-1 shadow-sm"
      ...
      <div className="mx-auto" style={{ maxWidth: "1400px" }}>
          ...
          {listings.map((listing, index) => {
            ...
            return (
              <ListingCard
                ...
                listing={listing}
                innerRef={isLastElement ? lastListingElementRef : undefined}
        ...
        {loading && (
          <div className="text-center py-5">
            <div className="spinner-border text-excha-orange"></div>
          </div>
        )}
      </div>
      ...
      {showModal && (
        <div
          className="custom-modal-overlay d-flex align-items-center justify-content-center p-3 animate-fade-in"
          style={{
            position: "fixed",
            top: 0,
            left: 0,
        ...
        >
          <div
            className="custom-modal-content bg-white rounded-5 shadow-2xl animate-slide-up"
            style={{
              maxWidth: "520px",
              width: "100%",
              maxHeight: "90vh",
          ...
          >
            <div className="p-4 d-flex justify-content-between align-items-center border-bottom sticky-top bg-white">
              <div>
                ...
                <p className="text-muted small mb-0">
                  Votre annonce sera visible sur le marché
                </p>
              </div>
              <button
            ...
            <form className="p-4" onSubmit={handleSubmit}>
              ...
              <div className="row g-3">
                ...
                <div className="col-6">
                  ...
                  <Select
                    ...
                    value={currencies.find(
                      (c) => c.value === formData.currency_from,
                    )}
                    onChange={(opt: any) =>
                      setFormData({ ...formData, currency_from: opt.value })
                    }
                  />
                </div>
                <div className="col-6">
                  ...
                  <Select
                    ...
                    value={currencies.find(
                      (c) => c.value === formData.currency_to,
                    )}
                    onChange={(opt: any) =>
                      setFormData({ ...formData, currency_to: opt.value })
                    }
                  />
                </div>
                ...
                <div className="col-12 text-center">
                  <div className="p-2 rounded-4 small fw-bold bg-light border text-muted transition-all">
                    {rateLoading ? (
                      ...
                      <span>
                        Taux marché : 1 {formData.currency_from} ={" "}
                        <strong>{marketRate || "..."}</strong>{" "}
                        {formData.currency_to}
                      </span>
                    )}
                  </div>
                ...
                <div className="col-12">
                  <label className="form-label small fw-bold">
                    Montant à vendre ({formData.currency_from})
                  </label>
                  <input
                    type="number"
                    min="0.01"
                    ...
                    onChange={(e) => {
                      setFormData({
                        ...formData,
                        amount_available: e.target.value,
                      });
                      setFormError(null);
                    }}
                ...
                <div className="col-12">
                  <label className="form-label small fw-bold">
                    Votre Taux personnalisé (Optionnel)
                  </label>
                  <input
                    type="number"
                    step="any"
                ...
                <div className="col-12">
                  <div
                    className="calculation-preview p-3 rounded-4 d-flex align-items-center justify-content-between shadow-sm"
                    style={{
                      backgroundColor: "#fff9f5",
                      border: "1px dashed #fd7e14",
                    }}
                  >
                    <div className="icon-circle bg-white shadow-sm p-2 rounded-circle me-3">
                      <MdTrendingUp size={24} className="text-success" />
                    </div>
                    <div className="flex-grow-1 text-end">
                      ...
                      <span className="fw-bolder fs-4 text-excha-orange">
                        {estimatedTotal} {formData.currency_to}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <button
                type="submit"
                className="btn btn-excha-orange w-100 rounded-pill mt-4 py-3 fw-bold text-white shadow-lg btn-publish-main"
              >
